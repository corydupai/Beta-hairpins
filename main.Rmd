...
ss_data_in <- "data_mid/pair_data_bigger.csv"
...
```{r manipulate_dataframes, include = FALSE}
...
```{r load_and_format_angles_dt}
turn_ss_dt <- 
  unique(
    pairs[,c("id", "full_aa", "full_ss",
             "subseq_aa", "subseq_ss",
             "touching_u","beta_type")]
    )[!grepl("X", full_aa)]
turn_ss_dt[, hidden_space := "NO",
    ][, turn_length := nchar(subseq_aa)
    ][ turn_length != nchar(subseq_ss), hidden_space := "YES"
    ][, turn_type := "Mixed"
    ][grepl("T", subseq_ss) & 
        !grepl("S", subseq_ss) & 
        hidden_space != "YES",
...
table(turn_ss_dt$turn_type)

# Loads and formats dt containing angle info
angles_in <- fread("data_mid/angles_info.csv"
                   )[PSI > 180, PSI := 360-PSI
                   ][PHI > 180, PHI := 360-PHI
                   ][touching_u == "YES"
                   ]
...
angles_in <- angles_in %>%
  select(-Letter) %>%
  unique() %>%
  pivot_wider(names_from = t_res_num,
              values_from = c("PSI","PHI","ALPHA","KAPPA","TCO"),
              names_glue = "{.value}_{t_res_num}") %>%
  left_join(unique(turn_ss_dt[grepl("Bonded", DSSP_type)][,c("id", "subseq_aa", "beta_type")])) %>%
  filter(!grepl("3",beta_type) & !is.na(beta_type)) # Remove sequences with odd beta strand contacts or missing contacts

angles_in <- classify_turns(angles_in)
angles_counts <- turn_breakdown(angles_in)
```

```{r}
# USE THIS GUY
beta_turn_plot <-
  ggplot(angles_counts %>% filter(TOTES >= 100 )) + #& grepl("turn_4",turn_type)
  geom_col( aes(x = structure, y = Perc, fill = AA)) +
  facet_grid(AA_group ~ full_turn_info, scales = "free_x") +
  scale_fill_manual(values = color_pal)

angles_out <- angles_counts %>%
  mutate(Perc = Perc * 100) %>%
  select(AA, Perc, "Turn Length" = turn_type, "Turn Type" = turn_subtype,"Turn Position" = structure, "Number of Hairpin structures" = TOTES) %>%
  pivot_wider(names_from = AA,
              values_from = Perc) %>%
  relocate(A:Y, .after = `Turn Position`) %>%
  relocate(M, .after = L) %>%
  relocate(W, .before = Y) %>%
  mutate(`Turn Length` = str_remove(`Turn Length`, "turn_"),
         `Turn Type` = str_replace(`Turn Type`, "_", " "))


fwrite(angles_out, "data/AA_frequency_by_type.csv")
```
...
```{r sec_structure_differences, include = FALSE}

counterzzz <- turn_breakdown(turn_ss_dt) %>%
  mutate(turn_type = str_remove(turn_type, "turn_"),
         turn_type = paste0(turn_type, " ", turn_subtype),
         width = if_else(grepl(" 1", turn_type), 10, 1))

# This adds some space around the 1 turn column
filler_plot <- data.frame(structure = c(0,2), 
                          Perc = 40, 
                          AA_simpler = "Charged", 
                          turn_type = "1 Bend", 
                          AA = "R")
ss_turn_plot <-
  ggplot(counterzzz %>% filter(TOTES >= 100)) +
  geom_col( aes(x = structure, y = Perc*100, 
                fill = AA,
              group = AA)) +
  geom_point(data = filler_plot, aes(x = structure, y = Perc), size = 15, colour = "white", alpha = 0)+
  facet_grid(AA_simpler ~ turn_type, scales = "free_x", space = "free_x") +
  scale_fill_manual(values = color_pal) + 
  scale_y_continuous(expand = c(0,0), name = "Frequency") +
  # scale_x_discrete(expand = c(1,1))+
  # scale_x_discrete(breaks = c(0,1,1.5,2,3,4,5), labels = c("",1, "", 2, 3, 4, 5))+
  theme(legend.position = "None",
        axis.line = element_blank(),
      panel.border = element_rect(colour = "black",
                                  fill = NA),
...
```{r alignment_stuff}
pairs2 <- fread("data_mid/pair_data_bigger.csv",
               na.strings=getOption("datatable.na.strings")
)[!(pdb_id %in% anti_ids)
][touching_u %in% c("YES","NO") &
    B1_pos != "-" &
